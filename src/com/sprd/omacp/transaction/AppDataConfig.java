...
class AppDataConfig {
    ...
    public int process(Context context, OtaMapData mapData) {
        ...
        if (Define.STATE_OK != vowifiDataConfig(context, mapData)) {
            Log.d(TAG, "vowifiDataConfig process error");
        }
    ...
    private int vowifiDataConfig(Context context, OtaMapData mapData) {
        int index = 0;
        List<MyHashMap> vowifiDatas = mapData.get(Define.TYPE_VOWIFI);
        if (vowifiDatas == null || vowifiDatas.isEmpty()) {
            Log.d(TAG, "vowifiDatas is null or empty");
            return Define.STATE_PARAM_ERROR;
        }
        Intent vowifiIntent = new Intent(Define.ACTION_VOWIFI);
        for (MyHashMap vowifiMap : vowifiDatas) {
            ++index;
            Bundle vowifiBundle = new Bundle();
            Set<String> vowifiSet = vowifiMap.keySet();
            for (String key : vowifiSet) {
                vowifiBundle.putString(key, vowifiMap.get(key));
            }
            //if (Debug.isDebug()) {
                Log.d(TAG, "[vowifiData" + index + "]=[" + vowifiBundle.toString() + "]");
            //}
            vowifiIntent.putExtra("vowifiData" + index, vowifiBundle);
        }
        vowifiIntent.putExtra("vowifi_size", vowifiDatas.size());
        vowifiIntent.putExtra("subid", OmacpUtils.getSubId());
        vowifiIntent.putExtra("version", Define.OMA_VERSION);
        context.sendOrderedBroadcast(vowifiIntent, null);
        Log.d(TAG, "send vowifi boardcast success!");
        index = 0;
        return Define.STATE_OK;
    }
    // config Email Account data and send broadcast
    private int emailDataConfig(Context context, OtaMapData mapData) {
        ...
        emailIntent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
    ...
    private int browseDataConfig(Context context, OtaMapData mapData) {
        ...
        browseIntent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
    ...
    private int apnDataConfig(Context context, OtaMapData mapData) {
        ...
        apnIntent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
