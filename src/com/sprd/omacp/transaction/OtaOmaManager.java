...
import com.sprd.process.list2hashmap.VowifiDataHandler;
...
public class OtaOmaManager {
    ...
    public int parse(Context context, String inputPin) {
        ...
        if (ret == Define.STATE_OK) {
            ...
            int ret_parse = getOtaParser().parse(context);
            if (ret_parse != Define.STATE_OK) {
    ...
    private int list2Map() {
        ...
        handlerList.add(new VowifiDataHandler());
    ...
    private void Debug() {
        ...
        DebugVisitor dv = new DebugVisitor();
        if(DefaultHandler.getNodeIns() != null){
                if(DefaultHandler.getInstance() != null){
                  DefaultHandler.getNodeIns().visitor(DefaultHandler.getInstance().getRoot(), param, dv);
                }
        }
        if((List2MapMgr.getInstance() != null) && (List2MapMgr.getInstance().getOtaMapData() != null)){
    ...
    private void checkKeys() {
        ...
        System.out
                .println("\n<<<------------------Check Vowifi Elements begin----------------------");
        for (MyHashMap myHashMap : List2MapMgr.getInstance().getOtaMapData()
                .get(Define.TYPE_VOWIFI)) {
            System.out.println("<<<----------------------item------------------------");
            List2MapMgr.getInstance().getOtaMapData().DebugHashMap(myHashMap);
            System.out.println("\n-----------------------item---------------------->>>");
            int nRet = ElementsMgr.CreateInstance().getList().get(ISet.OTA_VOWIFI)
                    .check(myHashMap.keySet());
            System.out.println("\nnRet = " + Integer.toHexString(nRet));
        }
        System.out
                .println("\n----------------Check Vowifi Elements end--------------------------->>>");
    }
    ...
    public boolean checkIsNetwpin() {
        return getOtaPreParser().isNetwpin();
    }
